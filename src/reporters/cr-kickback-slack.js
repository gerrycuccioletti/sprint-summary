// src/reporters/cr-kickback-slack.js
...
async function sendCRKickbackSlack(results, fromDate, toDate) {
  const webhookUrl = process.env.SLACK_WEBHOOK_DEFAULT || process.env.SLACK_WEBHOOK_URL;
  if (!webhookUrl) { console.warn('[CRKickback] No SLACK_WEBHOOK_DEFAULT set'); return; }
  ...
  blocks.push({ type: 'header', text: { type: 'plain_text', emoji: true,
    text: `${hEmoji} CR Kickback Ratio Report` }});

  blocks.push({ type: 'section', text: { type: 'mrkdwn',
    text: `*Period:* ${fromDate} → ${toDate}\n*Code Review → Dev In Progress kickbacks*` }});
  ...
  const rows = results.map(r => {
    ...
    return `\`${r.projectKey.padEnd(10)} ${miniBar(r.ratio)}  →CR:${String(r.denominator).padStart(3)}  InCR:${String(r.denominator2 || 0).padStart(3)}  ←:${String(r.numerator).padStart(3)}  R1:${String(r.ratio + '%').padStart(5)} ${e1}  R2:${String((r.ratio2 || 0) + '%').padStart(5)} ${e2}\``;
  }).join('\n');
  ...
  blocks.push({ type: 'section', fields: [
    { type: 'mrkdwn', text: `*→ Code Review (D1)*\n${total.denominator} tickets` },
    { type: 'mrkdwn', text: `*In Code Review (D2)*\n${total.denominator2} tickets` },
    ...
    { type: 'mrkdwn', text: `*Ratio 2* _(back/in CR)_\n${ratioEmoji(totalRatio2)} ${totalRatio2}%` },
  ]});

  blocks.push({ type: 'section', text: { type: 'mrkdwn',
    text: `_D1 = tickets that moved TO Code Review · D2 = tickets IN Code Review at any point · R1 = kickbacks/D1 · R2 = kickbacks/D2_` }});

  blocks.push({ type: 'divider' });

  // Kicked back detail per project
  ...
  blocks.push({ type: 'context', elements: [{ type: 'mrkdwn',
    text: `CR Kickback Ratio · ${new Date().toLocaleString()} · Jira Data Center` }]});

  await axios.post(webhookUrl, {
    text: `${hEmoji} CR Kickback Ratio — ${fromDate} → ${toDate} — R1: ${totalRatio}% · R2: ${totalRatio2}%`,
    blocks,
  }, { headers: { 'Content-Type': 'application/json' }});
}

module.exports = { sendCRKickbackSlack };
